...
// Fix for Node.js: CDN URL cannot be require()'d in Node.js.
// Use the local worker file instead (works for both polling and Vercel).
...
async function processSfg(buffer, onProgress) {
  try {
    ...
    const answersMatched   = (formatted.match(/\u2705/g) || []).length;
    const explanFound      = (formatted.match(/^Ex:/gm) || []).length;
    ...
    return {
      ...
      lineCount: lines,
      stats: {
        matched:           answersMatched,
        noAns:             qCount - answersMatched,
        explanationsFound: explanFound,
        noExpl:            qCount - explanFound,
      },
    };
  } catch (err) {
    return {
